defmodule PatternMatching do
  ...
  koan "The pattern can make assertions about what it expects" do
    assert match?([1, _second, _third], [1, 99, 100])
  end
  ...
  koan "Functions perform pattern matching on their arguments" do
    ...
    assert make_noise(cat) === ~s(Meow)
    assert make_noise(dog) === ~s(Woof)
  ...
  koan "A variable can be pinned to use its value when matching instead of binding to a new value" do
    ...
    example = fn
      ^pinned_variable -> ~s(The number One)
      2 -> ~s(The number Two)
      number -> ~s(The number #{number})
    end

    assert example.(1) === ~s(The number One)
    assert example.(2) === ~s(The number Two)
    assert example.(3) === ~s(The number 3)
  end
